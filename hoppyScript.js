...
function playSfx(a) {
	if (VOLUME_LEVELS[volumeLevel] === 0) return; // mute
	try {
		a.currentTime = 0;
		a.play();
	} catch (_) {}
...
// ------------------------- Audio Button ----------------------- //
const VOLUME_LEVELS = [0, 0.3, 0.6];

let volumeLevel = Number(localStorage.getItem("flappy_vol") || "1"); //default volume
if (!Number.isFinite(volumeLevel)) volumeLevel = 2;
volumeLevel = Math.max(0, Math.min(2, Math.floor(volumeLevel)));

function applyMasterVolume() {
	const v = VOLUME_LEVELS[volumeLevel];
	for (const a of Object.values(sfx)) {
		a.volume = v;
		a.muted = (v === 0);
	}
...
function cycleVolume() {
	...
	localStorage.setItem("flappy_vol", String(volumeLevel));
	applyMasterVolume();
}

const volumeBtn = { x: W - 8 - 28, y: 8, w: 28, h: 28 };
function drawVolumeButton() {
	...
	if (volumeLevel === 0) {
		...
		ctx.lineWidth = 1.6;
		ctx.beginPath();
		ctx.arc(10, 6, 4, -0.7, 0.7);
		ctx.stroke();
		if (volumeLevel >= 2) {
			ctx.beginPath();
			ctx.arc(10, 6, 6.5, -0.6, 0.6);
			ctx.stroke();
		}
	}
	ctx.restore();
...
canvas.addEventListener("mousedown", (e) => {
	...
	handlePointerAction(p.x, p.y);
});
canvas.addEventListener("touchstart", (e) => {
	e.preventDefault();
	const t = e.changedTouches[0];
	const p = getCanvasPosFromClient(t.clientX, t.clientY);
	handlePointerAction(p.x, p.y);
}, { passive: false });
...
function drawScoreboard() {
	...
	// draw number
	drawNumber(score, x + panelW - 28, y + 37, { align: "right", scale: 0.6, spacing: 1 });
	drawNumber(hiScore, x + panelW - 28, y + 82, { align: "right", scale: 0.6, spacing: 1 });
	...
	if (state === State.OVER) {
		...
		const btnH = 27;
		const btnX = (W - btnW) / 2;
		const btnY = y + panelH + 10;
		
		ctx.drawImage(images.button, btnX, btnY, btnW, btnH);
...
function handlePointerAction(gameX, gameY) {
	if (pointInRect(gameX, gameY, volumeBtn)) {
		cycleVolume();
		return;
	}
	
	if (state === State.OVER && death.boardActive && playAgainBtn && pointInRect(gameX, gameY, playAgainBtn)) {
		playSfx(sfx.swoosh);
		reset();
		return;
	}
